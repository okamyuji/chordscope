...
├── tests/                     # pytest (44 件: ユニット 26 + 統合 18)
...
uv run mypy src
uv run pytest -m "not integration"            # 高速ユニット (26 件、約 5 秒)
uv run pytest -m "integration"                # 統合 (18 件、約 3 分: PD 音源 + AST モデル DL)
uv run pytest                                 # 全 44 件
...
| mypy --strict | 0 issues in 23 source files |
| pytest 全 44 件 | 44 passed (ユニット 26 + 統合 18) |
